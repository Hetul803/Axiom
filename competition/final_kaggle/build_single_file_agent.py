...
import hashlib, json, subprocess, textwrap, time
from pathlib import Path
ROOT=Path(__file__).resolve().parents[2]
OUT=ROOT/'competition/final_kaggle/my_agent.py'
MANIFEST=ROOT/'competition/final_kaggle/agent_manifest.json'
SRC_FILES=[
 'axiom_core/types.py','axiom_core/config.py','axiom_core/perception/components.py','axiom_core/perception/scene_graph.py','axiom_core/perception/tracking.py','axiom_core/perception/transitions.py','axiom_core/causality/action_semantics.py','axiom_core/causality/interpreter.py','axiom_core/causality/hypotheses.py','axiom_core/causality/particles.py','axiom_core/causality/rule_induction.py','axiom_core/exploration/information_gain.py','axiom_core/exploration/novelty.py','axiom_core/exploration/reversibility.py','axiom_core/exploration/risk.py','axiom_core/exploration/selector.py','axiom_core/goals/induction.py','axiom_core/goals/posterior.py','axiom_core/planning/search.py','axiom_core/runtime.py','axiom_core/state_graph.py','axiom_core/diagnostics.py','axiom_core/roles.py','axiom_core/mechanics.py','competition/agent/my_agent.py']
TEMPLATE = r'''
from __future__ import annotations
...
def main():
    ...
    agent_sha=hashlib.sha256(OUT.read_bytes()).hexdigest()
    src_hashes={p:hashlib.sha256((ROOT/p).read_bytes()).hexdigest() for p in SRC_FILES if (ROOT/p).exists()}
    try: commit=subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip()
    except Exception: commit='unknown'
    MANIFEST.write_text(json.dumps({'generated_at':time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()),'source_commit':commit,'agent_sha256':agent_sha,'source_hashes':src_hashes,'included_source_count':len(src_hashes)},indent=2,sort_keys=True))
    print(json.dumps({'agent':str(OUT),'sha256':agent_sha,'lines':len(OUT.read_text().splitlines())},sort_keys=True))
